...
#' @import RColorBrewer
#' @import stats
nicepairs = function (x, punktcol = grDevices::rgb(0, 0, 0, 0.3), punktform = 4, cortype = "spearman",smoothness = 0.95, ...)
{
  ...
  panel.cor = function(x, y, digits = 2, prefix = "") {
    ...
    pval = try(stats::cor.test(x[isna], y[isna], method = cortype)$p.value, silent = T)
    ...
    graphics::text(0.5, 0.5, txt, cex = cex_todo, col = ifelse(is.na(r), "grey99", ifelse(r <
                                                                                  0, "dodgerblue4", "black")))
  }
